function [xPoints, yPoints] = plotSpikeRaster(spikes,omitted_trials, varargin)
...
if islogical(spikes)
    ...
    switch plotType
        ...
        case 'vertline'
            ...
            % Find the trial (yPoints) and timebin (xPoints) of each spike
            [trials,timebins] = find(spikes);
            if ~isempty(omitted_trials)
                IX = ismember(trials, str2num(omitted_trials{1}));
                trials_in_black = trials(IX)';
                timebins_in_black = timebins(IX)';
                trials = trials(~IX);
                timebins = timebins(~IX);
            end
            trials = trials';
            timebins = timebins';
            ...
            plot(xPoints,yPoints,'k',lineFormat{:});
            if ~isempty(omitted_trials)
                xPoints = [ timebins_in_black + relSpikeStartTime;
                            timebins_in_black + relSpikeStartTime;
                            NaN(size(timebins_in_black)) ];
                yPoints = [ trials_in_black - halfSpikeHeight + vertSpikePosition;
                            trials_in_black + halfSpikeHeight + vertSpikePosition;
                            NaN(size(trials_in_black)) ];
